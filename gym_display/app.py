...
def get_zone_color(hr, max_hr):
    ...
    pct = (hr / max_hr) * 100
    if pct < 60: return "#3b82f6"   # Bleu (Repos / Échauffement)
    elif pct < 70: return "#10b981" # Vert (Brûle-graisse)
    elif pct < 80: return "#f59e0b" # Jaune (Aérobie)
    elif pct < 90: return "#ef4444" # Rouge (Seuil anaérobie)
    else: return "#9333ea"          # Violet (Max / Alerte)

async def ble_reader_task():
    """Tâche asynchrone pour lire les capteurs Bluetooth des participants"""
    clients = {}
    while True:
        with thread_lock:
            active_participants = list(timer_state["participants"])
        
        for p in active_participants:
            mac = p.get("mac")
            if not mac: continue
            
            # Si non connecté, on essaie de se connecter
            if mac not in clients:
                try:
                    ...
                    if client.is_connected:
                        ...
                        await client.start_notify(HR_MEASUREMENT_UUID, make_handler(p["id"]))
                except Exception:
                    if mac in clients:
                        del clients[mac]
...
def handle_update_colors(data):
    with thread_lock:
        if 'bg_color' in data: timer_state["bg_color"] = data['bg_color']
        if 'clock_color' in data: timer_state["clock_color"] = data['clock_color']
